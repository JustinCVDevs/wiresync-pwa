<script lang="ts">
	...
	async function handleSampleWagon() {
		...
		if (unsampledWagons.length === 0) {
			...
			goto(`/bosveld/processes/loading-station/sampling/wagons?shuntingTrainIds=${shuntingTrainIds.join(',')}&wagonIds=${linkedWagonIds.join(',')}`);
		}
	}
					...
					{#each filteredWagons as wagon}
						<div class="flex flex-col">
							...
							<div
								...
								on:click={() => goto(`/bosveld/processes/loading-station/sampling/wagons?wagonIdSimple=${wagon.wagonIdSimple}&wagonDbId=${wagon.id}&shuntingTrainIds=${shuntingTrainIds.join(',')}&wagonIds=${linkedWagonIds.join(',')}`)}
							>
