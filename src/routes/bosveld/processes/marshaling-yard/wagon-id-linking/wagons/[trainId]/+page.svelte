	...
	async function handleSubmit() {
		try {
			...
			if (shuntingTrain) {
				...
				setTimeout(() => {
					goto('/bosveld/processes/marshaling-yard/wagon-id-linking');
				}, 1500);
			}
		} catch (e: any) {
			console.error(e);
		...
		<div class="space-y-2">
			{#each filteredWagons as wagon, index (wagon.id)}
				<div
					...
					on:click={() => goto(`/bosveld/processes/marshaling-yard/wagon-id-linking/wagons/${trainId}/edit/${wagon.id}?position=${index + 1}`)}
					on:keydown={(e) => e.key === 'Enter' && goto(`/bosveld/processes/marshaling-yard/wagon-id-linking/wagons/${trainId}/edit/${wagon.id}?position=${index + 1}`)}
				>
