<script lang="ts">
	...
	let materialType = 'LG';
	let sampleId = '';
	let loadingLocation = 'LG Concentrate';
	let error = '';
	...
	onMount(async () => {
		try {
			...
			trucks = (await indexedDBService.getAllRecords('trucks')).filter(
				(truck: Truck) => {
					const matchesProduct = truck.productType === 'LG' && !truck.sampleTimestamp;
	...
	async function handleSubmit() {
		try {
			...
			goto(
				`/pmc/processes/concentrator-&-smelter/unrefined-copper/lg-concentrate/sampling/verification?sampleId=${encodeURIComponent(sampleId)}&truckTransRef=${encodeURIComponent(transRef)}`
			);
		} catch (err) {
