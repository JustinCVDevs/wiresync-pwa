<script lang="ts">
	...
	async function handleSubmit() {
		...
		try {
			...
			// Aggregate linked wagons across selected trains
			let linkedWagonIds: string[] = [];
			for (const t of selectedTrains) {
				linkedWagonIds = linkedWagonIds.concat(t.linkedWagons || []);
			}

			// Use the first selected train as reference for navigation
			const refTrain = selectedTrains[0];
			...
			for (let wagonId of linkedWagonIds) {
				...
				if (wagon?.sampleTimestamp) {
					goto(`/pmc/processes/magnetite-rail/east-load-out/sampling/wagons/review?shuntingTrainIds=${selectedTrainIds.join(',')}&wagonIds=${linkedWagonIds.join(',')}`);
					return;
				}
			}

			goto(`/pmc/processes/magnetite-rail/east-load-out/sampling/wagons?shuntingTrainIds=${selectedTrainIds.join(',')}&wagonIds=${linkedWagonIds.join(',')}`);
		} finally {
			isSubmitting = false;
		}
	}
