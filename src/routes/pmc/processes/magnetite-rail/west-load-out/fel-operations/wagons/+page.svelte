	...
	async function handleSubmit() {
		...
		try {
			...
			if (selectedWagon) {
				...
				setTimeout(() => {
					goto(
						`/pmc/processes/magnetite-rail/west-load-out/fel-operations/wagons/review?wagonIdSimple=${encodeURIComponent(selectedWagon)}&shuntingTrainVerificationDate=${shuntingTrainVerificationDate}`
					);
				}, 1000);
			}
			// Clear persisted form data
