<script lang="ts">
	...
	onMount(async () => {
		...
		const shuntingTrains = (await indexedDBService.getAllRecords('shuntingTrains')).filter(
			shunting => {
				if (!shunting.verificationTimestamp || shunting.siteLocation !== 'PMC' || !shunting.created || shunting.finishFELOperationTimestamp) {
					return false;
				}
		...
		try {
			...
			for (let wagonId of linkedWagonIds) {
				...
				if (wagon?.felTimestamp) {
					goto(`/pmc/processes/magnetite-rail/west-load-out/fel-operations/wagons/review?shuntingTrainVerificationDate=${shuntingTrain.verificationTimestamp}`);
					isSubmitting = false;
					return;
				}
			}
			goto(`/pmc/processes/magnetite-rail/west-load-out/fel-operations/wagons?shuntingTrainVerificationDate=${shuntingTrain.verificationTimestamp}`);
		} finally {
			isSubmitting = false;
		}
	}
