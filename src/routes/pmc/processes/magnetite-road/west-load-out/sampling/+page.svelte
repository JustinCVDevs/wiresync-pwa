<script lang="ts">
	...
	async function getSampleNumberFromFleet() {
		...
		sampleNumberWest = ((existing?.lastNumber as number) || 0) + 1;
	...
	async function handleSubmit() {
		...
		try {
			...
			if (dedicatedFleet === 'Yes') {
				...
				await indexedDBService.saveRecord('deviceCounters', { id: `counter:${loadingLocation}:${todayStr}`, date: todayStr, location: loadingLocation, lastNumber: sampleNumberWest });
