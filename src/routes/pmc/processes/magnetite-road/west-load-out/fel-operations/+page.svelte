		...
		try {
			...
			if (dedicatedFleet === 'Yes') {
				...
				if (selectedTruck) {
					...
					goto(
						`/pmc/processes/magnetite-road/west-load-out/fel-operations/verification?truckRegistration=${encodeURIComponent(selectedTruck || '')}&sampleId=${encodeURIComponent(fleet?.sampleId || '')}`
					);
				}
			} else {
				...
				if (selectedTruck) {
					...
					const truckLoad = await indexedDBService.getAllRecords('truckLoads').then(loads => 
						loads.find(load => load.truckId === truck.serverId && load.loadingLocation === 'West Load Out')
					);
