<script lang="ts">
	...
	async function onVerify() {
	  ...
	  try {
		...
		setTimeout(() => goto('/processes'), 3000);
	  } catch (e) {
		console.error(e);
  ...
  >
	...
	{#if wagonData}
	  <div class="mb-6 rounded border border-gray-200 bg-gray-50 p-4">
		...
		<p class="mb-4 text-gray-700">Please verify below.</p>
		<div class="space-y-3">
		  <!-- RFID -->
		  <div class="flex items-center rounded border border-gray-200 bg-white px-3 py-2">
			<span class="w-48 text-gray-600 ">Wagon RFID Nr.</span>
			{#if isEditing}
			  <input
				name="rfidNumber"
				type="text"
				bind:value={wagonData.transcoreTag}
				class="animate-pulse bg-transparent focus:outline-none dark:text-gray-800"
			  />
			{:else}
			  <span class="font-medium dark:text-gray-800">{wagonData.transcoreTag}</span>
			{/if}
		  </div>
		  <!-- Wagon ID -->
		  <div class="flex items-center rounded border border-gray-200 bg-white px-3 py-2">
			<span class="w-48 text-gray-600">Wagon ID/Nr.</span>
			{#if isEditing}
			  <input
				name="wagonNumber"
				type="text"
			  ...
			  />
			{:else}
			  <span class="font-medium dark:text-gray-800">{wagonData.wagonIdSimple}</span>
			{/if}
		  </div>
		</div>
	  </div>
